...
export const setFormatedStatValue = (
  value: number,
  maxValue: number,
  minValue: number,
...
const PowerTracker = ({
  navigation,
  route,
}: NativeStackScreenProps<PowerTrackerParams, 'PowerTracker'>) => {
    ...
    <View style={styles.player_stats}>
      ...
      <View style={{ ...styles.player_stat, flex: 2 }}>
        ...
        <IconButton
          ...
          onPress={() => setHealBonusEditorVisible(true)}
          disabled={context.surges <= 0 || context.hp == context.maxHp}
        />
        ...
        <Text style={{ color: 'gray', fontSize: 10 }}>
          1/4MAX = {Math.floor(context.maxHp / 4.0)}
        </Text>
      </View>
      <View style={{ ...styles.player_stat, flex: 4 }}>
        <PlayerStat
  ...
  const Powers = sections.map(({ label, filterCriteria }, index) => (
    <List.Accordion title={label} key={label}>
      {arraySort(
        context.powers.filter(power => power.type.includes(filterCriteria)),
        ['level', 'action', 'name'],
      ).map((power: Power, ind: number) => (
        <PowerItem
          key={ind}
