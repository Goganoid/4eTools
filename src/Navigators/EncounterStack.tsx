...
export const EncounterStack = ({
  route,
}: {
  route:
    | RouteProp<GroupsStackParamList, 'Group'>
    | RouteProp<MainDrawerParamList, 'EncounterStack'>;
}) => {
  const theme = useTheme();
  if (route.params === undefined)
    throw 'No route passed to EncounterStackNavigator';
  const { mode, groupId } = route.params;
  const compendiumAddMode =
    mode == EncounterMode.encounter
      ? CategoryMode.encounter
      : CategoryMode.group;
  return (
    <Stack.Navigator
      ...
      <Stack.Screen
        ...
        options={({ route }) => {
          return configListingTitle(route, 'Add Monster');
        }}
      />
    </Stack.Navigator>
  );
